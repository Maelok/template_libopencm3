...
OUTPUT_BINARY_DIRECTORY=$(HOME)/mycoolrobot/myproject
JLINK_ROOT      := $(HOME)/JLink_Linux
JLINK           := $(JLINK_ROOT)/JLinkExe
JLINKFLAGS      := -If SWD -Device STM32F372CC -Speed 1000 -CommanderScript
## Program device
jlink-flash:
	$(JLINK) $(JLINKFLAGS) $(OUTPUT_BINARY_DIRECTORY)/flash.jlink

jlink-erase:
	$(JLINK) $(JLINKFLAGS) $(OUTPUT_BINARY_DIRECTORY)/erase-all.jlink

.PHONY: jlink-flash jlink-erase
